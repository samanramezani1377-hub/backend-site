...
function woogit_render_theme_management() {
    ...
    $features = woogit_theme_items('features_json', [['title'=>'امنیت و اعتماد','description'=>'مرز روشن بین Theme، Backend و فروشگاه مشتری.','icon'=>'◈','enabled'=>true]]);
    $steps = woogit_theme_items('steps_json', [['title'=>'شروع وب','description'=>'حساب خود را از مسیر رسمی وب وارد یا ایجاد کنید.','enabled'=>true]]);
    $faq = woogit_theme_items('faq_json', [['question'=>'آیا Theme به WooCommerce فروشگاه من متصل می‌شود؟','answer'=>'خیر. Theme فقط presentation و orchestration وب است.','enabled'=>true]]);
    $social = woogit_theme_items('social_links_json', []);
    ?>
    <div class="wg-tm">
      <div class="wg-tm__head"><div><h1>WooGit Theme</h1><p>مدیریت Presentation و محتوای وب؛ بدون ورود به مسئولیت‌های Backend یا Store Dashboard.</p></div><div class="wg-tm__save"><button type="submit" form="wg-theme-form" class="button button-primary button-large">ذخیره تغییرات</button></div></div>

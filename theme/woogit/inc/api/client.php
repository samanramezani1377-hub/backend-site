...
function woogit_api_request($method, $path, $body=null, array $extra_headers=[]) {
  ...
  if (!in_array($method,$allowed,true)) return new WP_Error('invalid_method');
  $url = trailingslashit(rest_url('woogit/v1')) . ltrim($path,'/');
  $headers = array_merge([
    'Accept'=>'application/json',
    'X-WooGit-Client'=>'web',
    'X-WooGit-Client-Version'=>WOOGIT_THEME_VERSION,
    'X-WP-Nonce'=>wp_create_nonce('wp_rest'),
  ], $extra_headers);
  $session = woogit_web_session();
  if ($session) $headers['X-WooGit-Web-Session'] = $session;
  ...
  if ($code >= 400) {
    $err = is_array($data) && isset($data['code']) ? sanitize_key($data['code']) : 'api_error';
    return new WP_Error($err, 'WooGit request failed', ['status'=>$code,'data'=>$data]);
  }

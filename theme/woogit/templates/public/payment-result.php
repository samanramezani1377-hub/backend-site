...
if (!$authenticated) {
    ...
    $message = 'پرداخت شما با موفقیت ثبت شده و دسترسی حساب نیز فعال است.';
} elseif ($paid) {
    $state = 'pending';
    $title = 'پرداخت تأیید شد؛ فعال‌سازی در حال تکمیل است';
    $message = 'پرداخت ثبت شده اما فعال‌شدن دسترسی هنوز در حال تکمیل است. پرداخت را دوباره انجام ندهید.';
} elseif ($pending) {
    $state = 'pending';
    ...
    $message = 'نتیجه نهایی پرداخت قابل تأیید نیست. برای جلوگیری از پرداخت تکراری، ابتدا تاریخچه پرداخت را بررسی کنید.';
}

get_header();
?>
<section class="wg-section"><div class="wg-container"><div class="wg-card wg-payment-result">
<span class="wg-eyebrow">نتیجه پرداخت</span>

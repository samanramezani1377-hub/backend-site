...
final class LifecycleAdmin
{
    ...
    public function renderSessions(): void
    {
        ...
        global $wpdb;
        $accounts=$wpdb->prefix.'woogit_accounts'; $sites=$wpdb->prefix.'woogit_sites'; $web=$wpdb->prefix.'woogit_web_sessions'; $app=$wpdb->prefix.'woogit_sessions';
    ...
    public function renderTrials(): void
    {
        if (!current_user_can('manage_options')) wp_die('دسترسی غیرمجاز.');
        global $wpdb;
        $accounts=$wpdb->prefix.'woogit_accounts'; $sites=$wpdb->prefix.'woogit_sites'; $entitlements=$wpdb->prefix.'woogit_entitlements';
        ...
        $used=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$accounts} WHERE trial_used_at IS NOT NULL"); $active=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$entitlements} WHERE status='trial' AND (expires_at IS NULL OR expires_at>UTC_TIMESTAMP())");
        ?><div class="wrap woogit-admin"><h1>مدیریت Trialهای WooGit</h1><p>وضعیت مصرف Trial، Trial فعال و Entitlement مربوط به Trial را مدیریت کنید.</p><?php $this->notice($notice); ?><div class="woogit-admin-cards"><div><strong><?php echo esc_html($used); ?></strong><span>Trial مصرف‌شده</span></div><div><strong><?php echo esc_html($active); ?></strong><span>Trial فعال</span></div><div><strong>۱۵ روز</strong><span>مدت Trial V1</span></div></div><div class="woogit-admin-table-wrap"><table class="widefat striped"><thead><tr><th>Account</th><th>Site</th><th>وضعیت مصرف</th><th>Trial Entitlement</th><th>شروع</th><th>انقضا</th><th>عملیات</th></tr></thead><tbody><?php if(!$rows): ?><tr><td colspan="7">Trialی ثبت نشده است.</td></tr><?php else: foreach($rows as $r): ?><tr><td><strong>#<?php echo esc_html($r['account_id']); ?></strong><br><?php echo esc_html($r['email']?:'—'); ?></td><td><?php echo esc_html($r['host']?:'—'); ?></td><td><?php echo $r['trial_used_at']?'مصرف‌شده<br><small>'.esc_html($r['trial_used_at']).'</small>':'آزاد'; ?></td><td><?php echo $r['entitlement_id']?esc_html($r['status']):'ندارد'; ?></td><td><?php echo esc_html($r['starts_at']?:'—'); ?></td><td><?php echo esc_html($r['expires_at']?:'—'); ?></td><td><div class="woogit-action-stack"><form method="post"><?php wp_nonce_field(self::ACTION); ?><input type="hidden" name="woogit_lifecycle_action" value="reset_trial"><input type="hidden" name="account_id" value="<?php echo esc_attr($r['account_id']); ?>"><input type="hidden" name="site_id" value="<?php echo esc_attr($r['site_id']); ?>"><button class="button">بازنشانی Trial</button></form><?php if($r['entitlement_id']): ?><form method="post" onsubmit="return confirm('Trial این Account غیرفعال شود؟');"><?php wp_nonce_field(self::ACTION); ?><input type="hidden" name="woogit_lifecycle_action" value="revoke_trial"><input type="hidden" name="account_id" value="<?php echo esc_attr($r['account_id']); ?>"><input type="hidden" name="site_id" value="<?php echo esc_attr($r['site_id']); ?>"><button class="button button-link-delete">لغو Trial</button></form><?php endif; ?></div></td></tr><?php endforeach; endif; ?></tbody></table></div></div><?php
    }

    private function handleSessionAction():?array
    {
        if(($_SERVER['REQUEST_METHOD']??'')!=='POST'||empty($_POST['woogit_lifecycle_action'])) return null;
        if(!current_user_can('manage_options')) return ['error','دسترسی غیرمجاز.']; check_admin_referer(self::ACTION); global $wpdb;
        $action=sanitize_key((string)$_POST['woogit_lifecycle_action']);
        if($action==='purge_expired_sessions'){$n=0;foreach([$wpdb->prefix.'woogit_web_sessions',$wpdb->prefix.'woogit_sessions'] as $table){$result=$wpdb->query("DELETE FROM {$table} WHERE expires_at<=UTC_TIMESTAMP()");if($result!==false)$n+=(int)$result;}return ['success',$n.' Session منقضی پاک‌سازی شد.'];}
        if($action==='revoke_session'){$type=sanitize_key((string)($_POST['session_type']??''));$id=absint($_POST['session_id']??0);$table=$type==='web'?$wpdb->prefix.'woogit_web_sessions':($type==='app'?$wpdb->prefix.'woogit_sessions':'');if(!$table||!$id)return ['error','Session نامعتبر است.'];$ok=false!==$wpdb->update($table,['revoked_at'=>gmdate('Y-m-d H:i:s')],['id'=>$id],['%s'],['%d']);return [$ok?'success':'error',$ok?'Session لغو شد.':'لغو Session انجام نشد.'];}return null;
    }
    ...
    private function handleTrialAction():?array
    {
        if(($_SERVER['REQUEST_METHOD']??'')!=='POST'||empty($_POST['woogit_lifecycle_action'])) return null;
        if(!current_user_can('manage_options')) return ['error','دسترسی غیرمجاز.']; check_admin_referer(self::ACTION); global $wpdb;
        $accounts=$wpdb->prefix.'woogit_accounts';$entitlements=$wpdb->prefix.'woogit_entitlements';$accountId=absint($_POST['account_id']??0);$siteId=absint($_POST['site_id']??0);if(!$accountId)return ['error','Account نامعتبر است.'];$action=sanitize_key((string)$_POST['woogit_lifecycle_action']);$now=gmdate('Y-m-d H:i:s');
        if($action==='reset_trial'){$ok=false!==$wpdb->update($accounts,['trial_used_at'=>null,'updated_at'=>$now],['id'=>$accountId],['%s','%s'],['%d']);return [$ok?'success':'error',$ok?'Trial برای این Account دوباره آزاد شد.':'بازنشانی Trial انجام نشد.'];}
        if($action==='revoke_trial'){if(!$siteId)return ['error','Site نامعتبر است.'];$ok=false!==$wpdb->query($wpdb->prepare("UPDATE {$entitlements} SET status='revoked',updated_at=%s WHERE account_id=%d AND site_id=%d AND status='trial'",$now,$accountId,$siteId));return [$ok?'success':'error',$ok?'Trial غیرفعال شد.':'لغو Trial انجام نشد.'];}return null;
    }

    private function notice(?array $notice):void { if($notice) echo '<div class="notice notice-'.esc_attr($notice[0]).' is-dismissible"><p>'.esc_html($notice[1]).'</p></div>'; }
}

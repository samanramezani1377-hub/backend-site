...
final class BazaarBillingService
{
    ...
    public function verify(int $accountId, int $siteId, string $productId, string $purchaseToken, string $packageName): array
    {
        ...
        $configuredPackage = defined('WOOGIT_BAZAAR_PACKAGE_NAME') ? trim((string)WOOGIT_BAZAAR_PACKAGE_NAME) : '';
        ...
        if ($existing) {
            ...
            return ['ok' => true, 'already_processed' => true, 'expires_at' => $existing['expires_at'] ?: null];
        }
        ...
        if (!$expiry) $expiry = time() + max(0, $billing->durationDaysForBazaar($plan['product'], (int)$plan['variation_id'])) * DAY_IN_SECONDS;
        ...
        $inserted = $wpdb->insert($table, [
            ...
            'order_id' => isset($payload['orderId']) ? sanitize_text_field((string)$payload['orderId']) : '',
    ...
    private function validateSubscription(string $packageName, string $productId, string $purchaseToken): array
    {
        ...
        if ($accessToken === '') return ['ok' => false, 'code' => 'bazaar_authorization_unavailable'];
        $base = defined('WOOGIT_BAZAAR_API_BASE_URL') ? rtrim((string)WOOGIT_BAZAAR_API_BASE_URL, '/') : self::DEFAULT_BASE_URL;
        ...
        if ($status === 401) {
            ...
            $accessToken = $this->accessToken();
            if ($accessToken !== '') $response = wp_remote_get($base . $path . '?access_token=' . rawurlencode($accessToken), ['timeout' => 12, 'redirection' => 2]);
            $status = (int)wp_remote_retrieve_response_code($response);
        }
        if ($status < 200 || $status >= 300) return ['ok' => false, 'code' => 'bazaar_purchase_invalid'];
        $json = json_decode((string)wp_remote_retrieve_body($response), true);
        if (!is_array($json)) return ['ok' => false, 'code' => 'bazaar_invalid_response'];
    ...
    private function accessToken(): string
    {
        ...
        $clientId = defined('WOOGIT_BAZAAR_CLIENT_ID') ? (string)WOOGIT_BAZAAR_CLIENT_ID : '';
        $clientSecret = defined('WOOGIT_BAZAAR_CLIENT_SECRET') ? (string)WOOGIT_BAZAAR_CLIENT_SECRET : '';
        $refreshToken = defined('WOOGIT_BAZAAR_REFRESH_TOKEN') ? (string)WOOGIT_BAZAAR_REFRESH_TOKEN : '';
        ...
        if ($token !== '') set_transient(self::ACCESS_TRANSIENT, $token, max(60, $expires - 60));
        return $token;
    }

    private function purchaseState(array $payload): ?int
    {
        if (!array_key_exists('purchaseState', $payload)) return null;
        return is_numeric($payload['purchaseState']) ? (int)$payload['purchaseState'] : null;
    }

    private function expiryTimestamp(array $payload): int
    {
        foreach (['expiryTimeMillis','expiry_time_millis','expirationTimeMillis'] as $key) if (isset($payload[$key]) && is_numeric($payload[$key])) return (int)floor(((int)$payload[$key]) / 1000);
        foreach (['expires_at','expiryTime','expirationTime'] as $key) if (isset($payload[$key])) { $value = $payload[$key]; if (is_numeric($value)) return (int)$value > 20000000000 ? (int)floor(((int)$value)/1000) : (int)$value; $parsed = strtotime((string)$value); if ($parsed) return $parsed; }
        return 0;
    }
    ...
    private function purchaseTime(array $payload): string
    {
        ...
        if (is_numeric($value)) return gmdate('Y-m-d H:i:s', ((int)$value > 20000000000 ? (int)floor(((int)$value)/1000) : (int)$value));

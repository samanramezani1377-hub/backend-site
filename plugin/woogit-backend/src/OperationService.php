...
final class OperationService
{
    ...
    {
        global $wpdb;$table=$wpdb->prefix.'woogit_operations';$now=current_time('mysql',true);$ok=$wpdb->insert($table,['operation_id'=>$operationId,'account_id'=>$accountId,'site_id'=>$siteId,'idempotency_key'=>$key,'request_fingerprint'=>$fingerprint,'resource'=>$resource,'operation_path'=>$path,'method'=>strtoupper($method),'status'=>'pending','created_at'=>$now,'updated_at'=>$now,'expires_at'=>gmdate('Y-m-d H:i:s',time()+86400)],['%s','%d','%d','%s','%s','%s','%s','%s','%s','%s','%s','%s']);return $ok?['operation_id'=>$operationId,'status'=>'pending']:null;
    }
    ...
    {
        global $wpdb;$row=$wpdb->get_row($wpdb->prepare("SELECT operation_id,account_id,site_id,resource,operation_path,method,status,upstream_status,response_body,created_at,updated_at,expires_at FROM {$wpdb->prefix}woogit_operations WHERE operation_id=%s LIMIT 1",$operationId),ARRAY_A);if(!$row)return null;$body=$row['response_body']!==null?json_decode($row['response_body'],true):null;$row['account_id']=(int)$row['account_id'];$row['site_id']=(int)$row['site_id'];$row['upstream_status']=$row['upstream_status']!==null?(int)$row['upstream_status']:null;$row['response_body']=is_array($body)?$body:null;return $row;
    }

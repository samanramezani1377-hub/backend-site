...
final class WooCommerceProxy
{
    ...
    {
        $isWordPressMedia=str_starts_with($path,'/wp-json/wp/v2/media');
        if(!$isWordPressMedia){$query['consumer_key']=$consumerKey;$query['consumer_secret']=$consumerSecret;}
        $url=$baseUrl.$path;
        if($query!==[])$url=add_query_arg($query,$url);

        $authorization=$isWordPressMedia
            ? 'Basic '.base64_encode($username.':'.$applicationPassword)
            : 'Basic '.base64_encode($consumerKey.':'.$consumerSecret);
        $headers=['Authorization'=>$authorization,'Accept'=>'application/json','User-Agent'=>'WooGit-Backend/'.WOOGIT_BACKEND_VERSION];

...
final class AccountService
{
    ...
    {
        global $wpdb;$table=$wpdb->prefix.'woogit_accounts';$row=$wpdb->get_row($wpdb->prepare("SELECT id,email,status FROM {$table} WHERE id=%d LIMIT 1",$accountId),ARRAY_A);return ($row&&$row['status']==='active')?$row:null;
    }
    ...
    public function create(string $email=''): ?array
    {
        ...
        $ok=$wpdb->insert($table,['email'=>$email!==''?$email:null,'status'=>'active','created_at'=>$now,'updated_at'=>$now],['%s','%s','%s','%s']);if(!$ok)return null;return ['id'=>(int)$wpdb->insert_id,'email'=>$email,'status'=>'active'];
    }

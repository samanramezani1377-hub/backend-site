...
final class EntitlementService
{
    ...
    public function grantTrial(int $accountId, int $siteId, int $days = 15): bool
    {
        ...
        return false !== $wpdb->insert($table, ['account_id'=>$accountId,'site_id'=>$siteId,'status'=>'trial','starts_at'=>$start,'expires_at'=>$end,'capabilities'=>wp_json_encode(['commerce'])], ['%d','%d','%s','%s','%s','%s']);
    }
}

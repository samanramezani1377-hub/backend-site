...
final class Database
{
    public static function install(): void
    {
        ...
        $accountSiteIndex = $wpdb->get_row($wpdb->prepare("SHOW INDEX FROM {$prefix}sites WHERE Key_name = %s", 'account_site'));
        if ($accountSiteIndex) $wpdb->query("ALTER TABLE {$prefix}sites DROP INDEX account_site");
        ...
        $wpdb->query("UPDATE {$prefix}idempotency SET state='pending' WHERE state='' OR state IS NULL");

...
final class AccountDeletionAdmin
{
    ...
    public function handle(): void
    {
        ...
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash((string)$_POST['_wpnonce'])), 'woogit_account_admin_action')) wp_die('درخواست نامعتبر است.');
        ...
        $tables=[
            $wpdb->prefix.'woogit_idempotency',
            $wpdb->prefix.'woogit_operations',
            $wpdb->prefix.'woogit_web_sessions',
            $wpdb->prefix.'woogit_sessions',
            $wpdb->prefix.'woogit_entitlements',
            $wpdb->prefix.'woogit_sites',
        ];
        foreach($tables as $table){
            $result=$wpdb->query($wpdb->prepare("DELETE FROM {$table} WHERE account_id=%d",$accountId));
            if($result===false)$this->finish('error','حذف کامل Account متوقف شد؛ حذف داده‌های مرتبط با یکی از جدول‌های WooGit ناموفق بود.');
        }
        ...
        $this->finish('success','Account، Site، Trial/Entitlement و تمام Sessionهای WooGit مرتبط با آن حذف شد. سفارش‌های WooCommerce حفظ شدند.');
    }

    private function finish(string $type,string $message): void
    {
        $url=add_query_arg(['page'=>'woogit-accounts','woogit_notice'=>$type,'woogit_notice_message'=>$message],admin_url('admin.php'));

...
final class RestController
{
    ...
    private const MAX_BODY_BYTES = 5242880;
    private const MAX_MEDIA_BYTES = 10485760;

    public function __construct(){ $this->sessions=new SessionService();$this->accounts=new AccountService();$this->sites=new SiteService();$this->entitlements=new EntitlementService();$this->idempotency=new IdempotencyService();$this->operations=new OperationService();$this->policy=new ProxyPolicy();$this->proxy=new WooCommerceProxy();$this->rateLimits=new RateLimitService();$this->versionGate=new VersionGate(); }
    ...
    public function verifySite(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $limit=$this->rateLimits->check('verify',$this->clientIp($request),10,60);if(!$limit['allowed'])return $this->rateLimited($limit['retry_after']);
        ...
        if($existingSite){
            ...
            $account=$this->accounts->get((int)$existingSite['account_id']);if(!$account)return new \WP_REST_Response(['code'=>'account_unavailable'],409);
            ...
            $account=$this->accounts->create($email);if(!$account)return new \WP_REST_Response(['code'=>'account_creation_failed'],500);
            $site=$this->sites->findOrCreate((int)$account['id'],$base);if(!$site)return new \WP_REST_Response(['code'=>'site_creation_failed'],500);
        }
    ...
    { $gate=$this->versionResponse($request);if($gate instanceof \WP_REST_Response)return $gate;return new \WP_REST_Response(['revoked'=>$this->sessions->revoke((string)$request->get_header('X-WooGit-Session'))],200); }
    ...
    public function getOperation(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $limit=$this->rateLimits->check('operation',(string)$session['account_id'].':'.(string)$session['site_id'],120,60);if(!$limit['allowed'])return $this->rateLimited($limit['retry_after']);
    ...
    public function forward(\WP_REST_Request $request): \WP_REST_Response
    {
        $gate=$this->versionResponse($request);if($gate instanceof \WP_REST_Response)return $gate;
        $session=$this->authenticateContext($request);if($session instanceof \WP_REST_Response)return $session;
        $limit=$this->rateLimits->check('forward',(string)$session['account_id'].':'.(string)$session['site_id'],120,60);if(!$limit['allowed'])return $this->rateLimited($limit['retry_after']);
        ...
        $method=strtoupper($request->get_method());$key=trim((string)$request->get_header('Idempotency-Key'));if($key!==''&&(strlen($key)>190||!preg_match('/^[A-Za-z0-9._:-]{1,190}$/',$key)))return new \WP_REST_Response(['code'=>'invalid_idempotency_key'],400);
        if(!$this->policy->validateMethod($method,$key!=='') )return new \WP_REST_Response(['code'=>'invalid_mutation_request'],400);
        ...
        $maxBody=str_starts_with($path,'/wp-json/wp/v2/media')?self::MAX_MEDIA_BYTES:self::MAX_BODY_BYTES;if(strlen($rawBody)>$maxBody)return new \WP_REST_Response(['code'=>'request_body_too_large','max_bytes'=>$maxBody],413);
        ...
        if($key!==''){
            ...
            if($candidate===null){
                $existing=$this->idempotency->lookup((int)$session['account_id'],(int)$session['site_id'],$key,$fingerprint);
                ...
                if($existing['state']==='pending')return new \WP_REST_Response(['code'=>'operation_in_progress','operation_id'=>$existing['operation_id'],'status'=>'pending','retryable'=>true],202);
                if($existing['state']==='unknown')return new \WP_REST_Response(['code'=>'operation_status_unknown','operation_id'=>$existing['operation_id'],'status'=>'unknown','retryable'=>false,'requires_reconciliation'=>true],409);
            ...
            if($claim['state']!=='claimed'){
                ...
                if($claim['state']==='pending')return new \WP_REST_Response(['code'=>'operation_in_progress','operation_id'=>$claim['operation_id'],'status'=>'pending','retryable'=>true],202);
                if($claim['state']==='unknown')return new \WP_REST_Response(['code'=>'operation_status_unknown','operation_id'=>$claim['operation_id'],'status'=>'unknown','retryable'=>false,'requires_reconciliation'=>true],409);
                return new \WP_REST_Response(['code'=>'operation_unavailable'],500);
            }
        }
        ...
        if($key!==''){
            if(!empty($result['timeout'])){$this->operations->markUnknown((int)$session['account_id'],(int)$session['site_id'],$operationId);$this->idempotency->markUnknown((int)$session['account_id'],(int)$session['site_id'],$key);return new \WP_REST_Response(['code'=>'upstream_timeout','operation_id'=>$operationId,'status'=>'unknown','retryable'=>false,'requires_reconciliation'=>true],504);}
    ...
    private function credentials(\WP_REST_Request $request,string $path): ?array
    {
        ...
        $headers=$isWordPressMedia?['X-WooGit-Wordpress-Username'=>'wordpress_username','X-WooGit-Wordpress-Application-Password'=>'wordpress_application_password']:['X-WooGit-Consumer-Key'=>'consumer_key','X-WooGit-Consumer-Secret'=>'consumer_secret'];
        $result=['wordpress_username'=>'','wordpress_application_password'=>'','consumer_key'=>'','consumer_secret'=>''];foreach($headers as $header=>$key){$value=(string)$request->get_header($header);if($value==='')return null;$result[$key]=$value;}return $result;
    }

    private function versionResponse(\WP_REST_Request $request): ?\WP_REST_Response
    {
        $result=$this->versionGate->check((string)$request->get_header('X-WooGit-App-Version'));if($result['allowed'])return null;
        $policy=$result['policy'];$status=$result['code']==='APP_VERSION_DEPRECATED'?426:400;
    ...
    { $response=new \WP_REST_Response(['code'=>'RATE_LIMITED','retry_after'=>$retryAfter,'retryable'=>true],429);$response->header('Retry-After',(string)$retryAfter);return $response; }

    private function clientIp(\WP_REST_Request $request): string
    { $ip=(string)$request->get_header('X-Forwarded-For');if($ip!=='')$ip=trim(explode(',',$ip)[0]);if(!filter_var($ip,FILTER_VALIDATE_IP))$ip=(string)$request->get_header('X-Real-IP');return filter_var($ip,FILTER_VALIDATE_IP)?$ip:'unknown'; }
}

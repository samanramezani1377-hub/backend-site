...
final class IdempotencyService
{
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'woogit_idempotency';
    ...
    {
        global $wpdb;
        $wpdb->update($wpdb->prefix.'woogit_idempotency', ['status_code'=>$status,'response_body'=>wp_json_encode($body, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'updated_at'=>current_time('mysql', true)], ['account_id'=>$accountId,'site_id'=>$siteId,'idempotency_key'=>$key], ['%d','%s','%s'], ['%d','%d','%s']);
    }
}

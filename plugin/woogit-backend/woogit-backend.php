...
foreach(['Database','IdentityService','AccountService','SiteService','EntitlementService','SessionService','WebSessionService','IdempotencyService','OperationService','ProxyPolicy','WooCommerceProxy','RateLimitService','VersionGate','VersionAdmin','AnnouncementService','AnnouncementAdmin','AnnouncementController','RequirementService','BillingService','TrialService','MiloBillingAdminCompatibility','ZarinPalPaymentBridge','WebSessionRestBridge','RestController','BillingController','WebAuthController','AccountAdmin'] as $file) require_once WOOGIT_BACKEND_DIR.'src/'.$file.'.php';
register_activation_hook(__FILE__,['WooGit\\Backend\\Database','install']);
add_action('plugins_loaded',static function():void{$from=(string)get_option('woogit_backend_db_version','');if($from !== WOOGIT_BACKEND_VERSION) \WooGit\Backend\Database::install($from);$trial=new \WooGit\Backend\TrialService();$trial->registerHooks();$billing=new \WooGit\Backend\BillingService();$billing->registerHooks();(new \WooGit\Backend\MiloBillingAdminCompatibility())->registerHooks();(new \WooGit\Backend\ZarinPalPaymentBridge())->register();(new \WooGit\Backend\WebSessionRestBridge())->register();});
add_action('admin_menu',static function():void{(new \WooGit\Backend\AccountAdmin())->register();(new \WooGit\Backend\VersionAdmin())->register();(new \WooGit\Backend\AnnouncementAdmin())->register();});

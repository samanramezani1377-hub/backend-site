...
define('WOOGIT_BACKEND_VERSION','0.4.1');
...
foreach(['Database','IdentityService','AccountService','SiteService','EntitlementService','SessionService','SessionPlanAdmin','WebSessionService','IdempotencyService','OperationService','ProxyPolicy','WooCommerceProxy','RateLimitService','VersionGate','VersionAdmin','AnnouncementService','AnnouncementAdmin','AnnouncementController','RequirementService','BillingService','BazaarBillingService','SettingsAdmin','TrialService','AutomaticTrialMiloBridge','MiloBillingAdminCompatibility','MiloEntitlementReconciler','ZarinPalPaymentBridge','ZarinPalPaymentStartBridge','PaymentReturnRedirect','WebSessionRestBridge','RestController','BillingController','WebAuthController','AccountsAdmin','LifecycleAdmin'] as $file) require_once WOOGIT_BACKEND_DIR.'src/'.$file.'.php';
register_activation_hook(__FILE__,['WooGit\\Backend\\Database','install']);
add_action('plugins_loaded',static function():void{$from=(string)get_option('woogit_backend_db_version','');if($from !== WOOGIT_BACKEND_VERSION) \WooGit\Backend\Database::install($from);$trial=new \WooGit\Backend\TrialService();$trial->registerHooks();$billing=new \WooGit\Backend\BillingService();$billing->registerHooks();(new \WooGit\Backend\SessionPlanAdmin())->registerHooks();(new \WooGit\Backend\AutomaticTrialMiloBridge())->registerHooks();(new \WooGit\Backend\MiloEntitlementReconciler())->registerHooks();(new \WooGit\Backend\MiloBillingAdminCompatibility())->registerHooks();(new \WooGit\Backend\ZarinPalPaymentBridge())->register();(new \WooGit\Backend\ZarinPalPaymentStartBridge())->register();(new \WooGit\Backend\PaymentReturnRedirect())->register();(new \WooGit\Backend\WebSessionRestBridge())->register();});
add_action('admin_menu',static function():void{
    ...
    $settings=new \WooGit\Backend\SettingsAdmin();

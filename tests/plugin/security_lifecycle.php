...
{
    public function __construct(public string $scope, public int $accountId, public int $siteId, public int $expiresAt, public bool $revoked = false) {}
    public function validAt(int $now): bool { return !$this->revoked && $this->expiresAt > $now; }
}
function authorizeForward(FakeSession $session, int $now, int $accountId, int $siteId, bool $entitled): bool
{
    return $session->validAt($now) && $session->scope === 'operational' && $session->accountId === $accountId && $session->siteId === $siteId && $entitled;
}
$now = 1_000;

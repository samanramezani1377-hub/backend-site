...
final class FakeCheckoutStore
{
    ...
    {
        if (!isset($this->records[$key])) { $this->records[$key] = ['fingerprint'=>$fingerprint,'state'=>'pending','order_id'=>null]; return ['status'=>'claimed']; }
        $record = $this->records[$key];
        if (!hash_equals($record['fingerprint'], $fingerprint)) return ['status'=>'conflict'];
        if ($record['state'] === 'pending') return ['status'=>'pending'];
        return ['status'=>$record['state'], 'order_id'=>$record['order_id']];
    }
    public function complete(string $key, int $orderId): void { $this->records[$key]['state']='succeeded'; $this->records[$key]['order_id']=$orderId; }
    public function unknown(string $key): void { $this->records[$key]['state']='unknown'; }
    public function state(string $key): string { return $this->records[$key]['state']; }
}

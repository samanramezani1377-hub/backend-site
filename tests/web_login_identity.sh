...
# satisfy the assertions accidentally. Do not depend on indentation because
# the controller source is intentionally compact.
...
# Use fixed-string checks: the PHP source contains literal "$" identifiers,
# which must not be interpreted as shell variables by grep.
